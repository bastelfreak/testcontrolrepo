plan profiles::test {
  ...
  $main_env = $agent_results.results[0].value['status']
  ...
  $puppetdb = puppetdb_query("nodes[catalog_environment]{ certname = \"${primary_host}\"}")
  $puppetdb_env = puppetdb_query[0]['catalog_environment']
